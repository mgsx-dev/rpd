...
After checking out the repo, run `bin/setup` (or `bin/setup --path vendor/bundle`) to install dependencies. Then, run `rake spec` (or `bundle exec rake spec`) to run the tests. You can also run `bin/console` for an interactive prompt that will allow you to experiment.
